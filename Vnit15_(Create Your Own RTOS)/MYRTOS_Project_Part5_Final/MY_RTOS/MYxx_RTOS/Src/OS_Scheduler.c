...
eOS_ErrorState MYRTOS_AcquireMutex(sOS_Mutex_ControlBlock* Mutex_TCB, sOS_Task_ControlBlock* Task_TCB)
{
	...
	// No Task Acquired The Mutex
	if( Mutex_TCB->CurrentMutex_Holder == NULL )
	{
		Mutex_TCB->CurrentMutex_Holder = Task_TCB;
	}
	else   // There is a Task Acquired The Mutex
	{
		if( Mutex_TCB->NextMutex_Holder == NULL )
		{
			Mutex_TCB->NextMutex_Holder = Task_TCB;
			// Update Task State
			Task_TCB->eOS_TaskState = TASK_SUSPEND;
			// Put Task in Suspended State
			MYRTOS_TerminateTask(Task_TCB);
		}
		else  // There is a Pending Request by another Task
		{
			err = MUTEX_REACHEDMAXIMUM_NUMBEROF_ACQUIREMENTS;
		}
	}
 	return err;
}
...
{
	// A Task Acquired The Mutex
	if( Mutex_TCB->CurrentMutex_Holder != NULL )
	{
		// Next Pending Task Acquire Mutex
		Mutex_TCB->CurrentMutex_Holder = Mutex_TCB->NextMutex_Holder;
		// Reset Next Pending Request
		Mutex_TCB->NextMutex_Holder = NULL;
		// Update Task State
		Mutex_TCB->CurrentMutex_Holder->eOS_TaskState = TASK_WAITING;
		// Put Task in Waiting State
		MYRTOS_ActivateTask(Mutex_TCB->CurrentMutex_Holder);
	}
}
